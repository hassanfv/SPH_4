...
def mainFunc(i, rkpc_i, Lsh_i):
  ...
  command = f"mpirun -np 6 -x LD_LIBRARY_PATH=/path/to/install/dir/lib:$LD_LIBRARY_PATH python3 chimes-driver.py {updated_file_name}"
  print(command)
  os.system(command)
  #----
...
for i in range(N_rkpc):
  for j in range(N_Lsh):
    ...
    if not file_exists:
      print("HASSAN: Failed to find file after waiting!!!!!!!!!!!!!!!!!!!!!!!!!!")
    else:
      print("HASSAN: File found, proceeding!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    #------------------------------------------------------------------------------------------------------
