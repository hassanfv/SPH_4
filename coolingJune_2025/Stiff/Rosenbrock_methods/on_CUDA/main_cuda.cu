...
int main()
{
  ...
  CHECK_CUDA(cudaMalloc(&d_y0, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_dydx, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_yscal, N_part * n * sizeof(float)));

  CHECK_CUDA(cudaMalloc(&d_a, N_con * n * n * sizeof(float)));  // 3D !
  CHECK_CUDA(cudaMalloc(&d_dfdy, N_con * n * n * sizeof(float))); // 3D !
  CHECK_CUDA(cudaMalloc(&d_indx, N_part * n * sizeof(int)));
  CHECK_CUDA(cudaMalloc(&d_dfdx, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_dysav, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_err, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_ysav, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_g1, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_g2, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_g3, N_part * n * sizeof(float)));
  CHECK_CUDA(cudaMalloc(&d_g4, N_part * n * sizeof(float)));
  ...
  int nBlocksPerGrid = 1;
  int nThreadsPerBlock = 32; // automate it !!
