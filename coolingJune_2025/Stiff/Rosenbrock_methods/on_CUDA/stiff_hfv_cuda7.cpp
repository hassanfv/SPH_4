...
// The difference with __6.cpp is that here we remove 'd' from ludcmp function.
// The difference with __5.cpp is that here we removed hdid.
// The difference with __4.cpp is that I removed ystart from odeint_h because it was needed only once in odeint_h and also stiff would always keep the original backed up !
...
// The difference with __2.cpp is that here we remove the x dependence, i.e. now it is dy/dx = f(y) but in _2.cpp it was dy/dx = f(x, y). We exclude
...
//----- ludcmp
void ludcmp(float *a, int *indx, int n) // void NR::ludcmp(Mat_IO_DP &a, Vec_O_INT &indx, DP &d)
{
...
void stiff(float *y, float *dydx, float &x, const float htry,
	const float eps, float *yscal, float &hnext, int n)
{
	...
	const float htry, const float hmin, int nvar) // htry is h1 in my other script if you are confused one day !!!!!!
{
	...
	for (nstp = 0;nstp < MAXSTP; nstp++) 
	{
		...
		stiff(y, dydx, x, h, eps, yscal, hnext, nvar); // y will be backed up in stiff so that its original value is always available in stiff !
